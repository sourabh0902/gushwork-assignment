    ...
    const handleScroll = () => {
        ...
        if (currentScrollY > foldHeight) {
            ...
            if (currentScrollY > lastScrollY) {
                ...
                // If header is appearing for the first time (no prior fixed/sticky state),
                // prime it with is-fixed (hidden above viewport) and force a reflow so the
                // transition from translateY(-100%) -> translateY(0) animates smoothly.
                if (!header.classList.contains('is-sticky') && !header.classList.contains('is-fixed')) {
                    header.classList.add('is-fixed');
                    // Force reflow to commit the hidden state before transitioning in.
